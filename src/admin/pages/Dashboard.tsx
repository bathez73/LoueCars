...
const Dashboard = () => {
  ...
  const stats = [
    ...
    { label: 'Véhicules', value: vehicules.length, icon: Car, color: 'from-[#F05A1A] to-[#D44D12]', link: '/admin/vehicules', badge: 0 },
    ...
    <div>
          ...
          <motion.div key={stat.label} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}>
            <Link to={stat.link} className="block bg-zinc-900 border border-zinc-800 rounded-2xl p-5 hover:border-zinc-700 transition-all group">
              ...
              <div className="flex items-center gap-2">
                ...
                {stat.badge > 0 && (
                  <span className="px-2 py-0.5 bg-[#F05A1A] text-white text-xs font-bold rounded-full">{stat.badge} non lu{stat.badge > 1 ? 's' : ''}</span>
                )}
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}
          ...
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-white font-semibold flex items-center gap-2">
              <TrendingUp className="w-5 h-5 text-[#F26831]" /> Véhicules récents
            </h2>
            <Link to="/admin/vehicules" className="text-[#F47A45] text-sm hover:text-[#F26831] transition-colors">Voir tout</Link>
          </div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}
          ...
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-white font-semibold flex items-center gap-2">
              <Mail className="w-5 h-5 text-[#F26831]" /> Messages récents
              {unreadMessages > 0 && <span className="px-2 py-0.5 bg-[#F05A1A] text-white text-xs font-bold rounded-full">{unreadMessages}</span>}
            </h2>
            <Link to="/admin/messages" className="text-[#F47A45] text-sm hover:text-[#F26831] transition-colors">Voir tout</Link>
          </div>
          <div className="space-y-3">
            {recentMessages.length === 0 ? (
              ...
              <Link key={msg.id} to="/admin/messages"
                ...
                <div className={`w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0 text-white font-bold text-sm ${!msg.read ? 'bg-[#F05A1A]' : 'bg-zinc-700'}`}>
                  {msg.name.charAt(0).toUpperCase()}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5">
                    <p className={`text-sm font-medium truncate ${!msg.read ? 'text-white' : 'text-gray-300'}`}>{msg.name}</p>
                    {!msg.read && <span className="w-1.5 h-1.5 bg-[#F26831] rounded-full flex-shrink-0" />}
                  </div>
